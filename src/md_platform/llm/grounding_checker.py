...
def extract_all_values_from_bundle(bundle: AnalysisBundle) -> List[float]:
    ...
    # Modules
    for module_name, mod_res in bundle.modules.items():
        for metric_name, scalar in mod_res.scalar_metrics.items():
...
def check_grounding(draft_report: str, bundle: AnalysisBundle) -> List[str]:
    """
    ...
    bundle_values = extract_all_values_from_bundle(bundle)
