"""
FastAPI application — MD AI Analyzer.

Serves the REST API, SSE progress stream, and the frontend SPA.
Includes security middleware (rate limiting, CORS, security headers,
request-ID tracing) and structured logging.

Changes from original:
- Added `from __future__ import annotations`.
- Typed all helpers and middleware.
- Extracted `_validate_file_extension` into a helper with clearer semantics.
...
from .config import (
    ALL_EXTENSIONS, CORS_ORIGINS, FRONTEND_DIR, MAX_UPLOAD_SIZE_BYTES,
    MAX_UPLOAD_SIZE_MB, RATE_LIMIT_REQUESTS, RATE_LIMIT_WINDOW_SECONDS,
    RESULTS_DIR, UPLOAD_DIR, get_system_info, logger,
)
from .models import AnalysisRequest, AnalysisStatus, ErrorResponse, UploadResponse
...
def _validate_job_id(job_id: str) -> str:
    """Validate *job_id* is an 8-char hex string, else raise 400."""
...
def _has_allowed_extension(filename: str, allowed: set[str]) -> bool:
...
class RequestIDMiddleware(BaseHTTPMiddleware):
    """Attach a unique request ID header for tracing."""
...
async def upload_files(
    ...
    for label, upload in [
        ...
        if upload and upload.filename:
            if not _has_allowed_extension(upload.filename, ALL_EXTENSIONS):
            ...
            logger.info(
                "Saved %s file: %s (%d bytes)", label, safe_name, len(content)
            )
    ...
    # Register job and reconcile directory naming
    ...
    logger.info(
        "Upload complete: job_id=%s, files=%s",
        actual_job_id, list(saved_files.keys()),
    )
...
async def start_analysis(request: AnalysisRequest) -> dict[str, str]:
    """Launch the analysis pipeline in the background."""
...
async def stream_progress(job_id: str) -> StreamingResponse:
    ...
    async def event_generator():
        ...
        try:
            # Send initial status
            yield (
                f"data: {json.dumps({'job_id': job_id, 'status': job['status'].value, 'progress_percent': job['progress'], 'current_module': job['current_module'], 'message': job['message']})}\n\n"
            )
            while True:
                try:
...
async def get_csv(job_id: str) -> FileResponse:
...
async def get_structure(job_id: str) -> FileResponse:
    """Serve the PDB/GRO structure file for the 3D viewer."""
    _validate_job_id(job_id)
    job = orchestrator.get_job(job_id)
    if not job:
        raise HTTPException(status_code=404, detail="Job not found")
...
async def get_pdf_report(job_id: str) -> FileResponse:
    """Export and serve the PDF report."""
    _validate_job_id(job_id)
    job = orchestrator.get_job(job_id)
    if not job or not job.get("result"):
    ...
    try:
        ...
        logger.error("PDF generation failed for job %s: %s", job_id, exc)
