...
def generate_html_report(result: Any, output_dir: Path) -> Path:
    ...
    displayed_time = (
        info.get("analyzed_time_ns")
        if info.get("subset_applied")
        else info.get("total_time_ns", 0)
    )
    displayed_time = float(displayed_time or 0.0)
    displayed_time_label = "Analyzed Time" if info.get("subset_applied") else "Total Time"
    ...
    plot_names_map: dict[str, str] = {
        ...
        "gnn_plot": "GNN Topological Ranking",
        "transformer_plot": "Transformer Change-Point Analysis",
        ...
        "binding_kinetics_plot": "Binding Contact Persistence",
    ...
    for ins in insights:
        ...
        <div class="insight-card" style="border-left: 4px solid {color};">
            <div class="insight-header">
                ...
                <span class="confidence-badge">Heuristic confidence: {confidence:.0%}</span>
            </div>
...
<body>
    <div class="container">
        ...
        <div class="info-grid">
            ...
            <div class="info-card">
                <div class="value">{displayed_time:.1f} ns</div>
                <div class="label">{displayed_time_label}</div>
            </div>
        </div>

        <h2>AI-Assisted Biological Hypotheses</h2>
        <p style="color:#999; margin-bottom: 12px;">Insight scores are heuristic and detector-specific. They are useful for prioritising follow-up checks, not as calibrated probabilities.</p>
        ...
        <footer style="text-align: center; color: #666; margin-top: 40px; padding: 20px; border-top: 1px solid #1a1a3e;">
            Generated by MD AI Analyzer | AI-assisted exploratory molecular dynamics analysis
        </footer>
    </div>
</body>
</html>"""
...
def export_csv(result: Any, output_dir: Path) -> Path:
    ...
    if isinstance(gnn, dict) and "residue_importance" in gnn:
        resids_gnn: list[Any] = gnn.get("resids", list(range(len(gnn["residue_importance"]))))
        for r, v in zip(resids_gnn, gnn["residue_importance"]):
    ...
    if isinstance(allo, dict) and "hub_residues" in allo:
        for h in allo["hub_residues"]:
            rows.append({
                "metric": "Allosteric_hub",
                "residue": h.get("resid"),
                "value": h.get("betweenness", 0),
                "unit": "betweenness",
            })
    ...
    if isinstance(bk, dict):
        if bk.get("kon_estimate_per_ps") is not None:
            rows.append({"metric": "BindingKinetics_kon", "value": bk["kon_estimate_per_ps"], "unit": "1/ps"})
        if bk.get("koff_estimate_per_ps") is not None:
            rows.append({"metric": "BindingKinetics_koff", "value": bk["koff_estimate_per_ps"], "unit": "1/ps"})
        if "contact_on_rate_proxy_per_ps" in bk:
            rows.append({"metric": "BindingKinetics_kon_proxy", "value": bk["contact_on_rate_proxy_per_ps"], "unit": "1/ps"})
        if "contact_off_rate_proxy_per_ps" in bk:
            rows.append({"metric": "BindingKinetics_koff_proxy", "value": bk["contact_off_rate_proxy_per_ps"], "unit": "1/ps"})
        rows.append({
